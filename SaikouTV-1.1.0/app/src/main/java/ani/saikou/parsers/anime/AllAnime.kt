...
class AllAnime : AnimeParser() {
    ...
    override val hostUrl = "https://api.anime-dex.workers.dev"
...
//            "mp4upload.com"               -> Mp4Upload(server)
    ...
    override suspend fun search(query: String): List<ShowResponse> {
        ...
        client.get("$hostUrl/search/$encoded").document
            .select(".last_episodes > ul > li div.img > a").forEach {
                val link = it.attr("link").toString().replace("/category/", "")
                val title = it.attr("title")
                val cover = it.select("img").attr("img")
